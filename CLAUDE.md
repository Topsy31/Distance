...
- `options.html` / `options.js` — settings page: units toggle (metric/imperial, saved instantly, unencrypted), a "Show return distance" checkbox (`showReturnDistance`, saved instantly, unencrypted, same pattern as units), passphrase field, API key input, home address input. **Save & encrypt** sends one `SAVE_VAULT` message (background geocodes the address, then encrypts `{apiKey, home}` under the passphrase). **Unlock & edit** (`UNLOCK`) decrypts the existing vault to pre-fill the fields. A "How do I get one?" link next to the API key field opens `#guide` — a right-hand slide-out panel (`.guide`, CSS `transform` transition) with a step-by-step Google Cloud walkthrough for obtaining the key; it's non-modal (`aria-modal="false"`), stays open until dismissed via the ✕, a click on the dimming overlay, or Escape, and scrolls independently so the form stays usable. Purely presentational — no storage or messaging. Mirrors the README's "Getting a Google Maps API key" prose.
...
Passphrase-encrypted vault in place (manifest, background worker, content script, options page, popup). Selection detection covers both regular page text and text selected inside `<input>`/`<textarea>` fields. Options page has an in-UI slide-out guide for obtaining the Google Maps API key. Icons in place at `icons/icon{16,32,48,128}.png` (analog odometer showing "31" with a mid-roll third digit, white on black), wired into both the manifest's top-level `icons` and `action.default_icon`; source images (`icon.png` — earlier red-car-on-blue design, `icon-odometer.png`, `icon-odometer-square.png`) are kept at the repo root for regenerating sizes if the design changes again. The mid-roll third digit is only legible at 128px — at 16-32px (actual toolbar size) it reads as an ambiguous mark, a known tradeoff accepted in favor of the odometer concept. No build/lint/test tooling exists; there's no bundler because the extension currently has no dependencies to bundle.
